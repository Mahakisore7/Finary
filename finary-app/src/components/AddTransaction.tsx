...
// Define the primary categories to match backend, dashboard, and budgets
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-white text-black hover:bg-zinc-200 font-black uppercase italic tracking-tighter transition-all active:scale-95 px-6 rounded-xl">
          <PlusCircle className="mr-2 h-4 w-4" /> 
          <span className="text-[10px] tracking-widest font-bold">Add Transaction</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="bg-zinc-950 border border-zinc-800 text-white shadow-2xl rounded-[2rem]">
        ...
        <form action={handleSubmit} className="space-y-6 pt-4">
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Amount (₹)</label>
            <Input 
              name="amount" 
              type="number" 
              step="0.01" 
          ...
          <div className="space-y-2">
            ...
            {/* Switched to Select to prevent manual entry errors */}
            <select 
              name="category" 
              required
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Description</label>
            <Input 
              name="description" 
              placeholder="Note (optional)" 
              className="bg-zinc-900/50 border-zinc-800 h-14 font-medium focus:ring-zinc-700 rounded-xl" 
            />
          </div>
          ...
          <Button 
            type="submit" 
            disabled={loading}
            className="w-full bg-emerald-500 text-black hover:bg-emerald-400 h-14 font-black text-lg transition-all rounded-xl uppercase italic tracking-tighter"
          >
            {loading ? (
